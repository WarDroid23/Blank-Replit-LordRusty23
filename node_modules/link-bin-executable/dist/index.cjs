...
const exists = file => promises.lstat(file).then(() => true, () => false);
let log = debug('linkbe');
const { platform } = process;
const windows = platform === 'win32';
async function findExeOnRun(name, scriptDirectory) {
  const exe = path.join(scriptDirectory, '..', windows ? `${name}.exe` : name);
  if (!await exists(exe)) {
    log('exe "%s"', exe);
...
function runExe(exe) {
  ...
  return new Promise((resolve, reject) =>
    child_process.spawn(exe, args, { stdio: 'inherit' })
      .on('error', reject)
      .on('exit', code => code ? reject(code) : resolve())
  )
}
async function findBinOnRun(scriptDirectory) {
  let bin = path.join(scriptDirectory, '..', '..', '.bin');
  log('local bin "%s"', bin);
  if (!await exists(bin)) {
    bin = path.join(scriptDirectory, '..', '..', '..', '..', 'bin');
    log('global bin "%s"', bin);
    if (!await exists(bin)) {
      bin = path.join(scriptDirectory, '..', 'node_modules', '.bin');
...
async function replaceSymlink(bin, name, exe) {
  ...
  log('stat "%s"', link);
  const { mode } = await promises.lstat(link);
  if (mode & 0o222) {
    log('unlink "%s"', link);
    await promises.unlink(link);
    log('link "%s"', link);
    await promises.symlink(exe, link);
    return true
  } else {
...
async function replaceCmd(bin, name, exe) {
  ...
  log('stat "%s"', cmd);
  const { mode } = await promises.lstat(cmd);
  if (mode & 0o222) {
    log('unlink "%s"', cmd);
    await promises.unlink(cmd);
    log('write "%s"', cmd);
    await promises.writeFile(cmd, `"${exe}" %*`);
    return true
  } else {
...
async function replacePs(bin, name, exe) {
  const ps = path.join(bin, `${name}.ps1`);
  log('stat "%s"', ps);
  const { mode } = await promises.lstat(ps);
  if (mode & 0o222) {
    log('unlink "%s"', ps);
    await promises.unlink(ps);
    log('write "%s"', ps);
    await promises.writeFile(ps, `#!/usr/bin/env pwsh
...
async function findExeOnInstall(name, packageDirectory) {
  const exe = path.join(packageDirectory, windows ? `${name}.exe` : name);
  if (!await exists(exe)) {
    log('exe "%s"', exe);
    throw new Error('missing executable')
  }
...
exports.runAndReplaceLink = runAndReplaceLink;
